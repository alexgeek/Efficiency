...
float raymarch(in vec3 from, in vec3 dir) 
{
	...
	vec3 p;
	float ra=rand(uv.xy*globaltime)-.5;
	float ras=max(0.,sign(-.5+rand(vec2(1.3456,.3573)*floor(30.+globaltime*20.))));
	float rab=rand(vec2(1.2439,2.3453)*floor(10.+globaltime*40.))*ras;
	float rac=rand(vec2(1.1347,1.0331)*floor(40.+globaltime));
	float ral=rand(1.+floor(uv.yy*300.)*globaltime)-.5;
	for (int i=0; i<60; i++) {
		p=from+totdist*dir;
		d=de(p);
	...
	if (rac>.7) col=col*.7+(.3+ra+ral*.5)*mod(uv.y+globaltime*2.,.25);
	col = mix(col, .5+ra+ral*.5, max(0.,3.-globaltime)/3.);
	return col+ra*.03+(ral*.1+ra*.1)*rab;
}

void main(void)
{
	float t=globaltime*.2;
	vec2 uv = position.xy;
